...
- Idea Maturity structural depth interpretation. Product Workspace now renders
  SpecGraph-owned `candidate_structure_review` readiness explainers next to the
  raw depth counts, so shallow candidate narratives can point operators toward
  candidate overview/topology review while Metrics remains objective telemetry.
