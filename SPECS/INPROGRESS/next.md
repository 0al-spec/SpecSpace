...
- Future artifact-backed slots: Feature Evidence and Agent Identity panels are planned only after SpecGraph publishes safe derived indexes for Feature Passport evidence and Agent Passport verification. Feature Evidence is specifically blocked on SpecGraph's `FP-RFC-0001` `0.2.0` producer schemas for passport indexes, receipt projections, and claim-evaluation results. Do not implement these panels against raw run/executor logs, raw telemetry, raw receipts, private passport material, local prompt/config files, or external RFC parsing.
