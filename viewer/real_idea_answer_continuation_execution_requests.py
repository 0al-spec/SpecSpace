"""SpecSpace-owned request state for controlled real idea answer continuation."""
...
def empty_state(path: Path | None = None) -> dict[str, Any]:
    return {
        ...
        "requests": [],
        "summary": {
            "status": "no_real_idea_answer_continuation_execution_requests",
            "request_count": 0,
            "requested_count": 0,
            "superseded_count": 0,
            "consumed_count": 0,
            ...
            "workspace_count": 0,
            "next_gap": "request_real_idea_answer_continuation_execution",
        },
        "consumer_boundary": {
            "specspace_owned_state": True,
            "for_real_idea_answer_continuation_execution": True,
            "public_safe": False,
        ...
        "authority_boundary": {
            "real_idea_answer_continuation_execution_request_state_is_authority": False,
...
def normalize_state(raw: Any, path: Path) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if raw.get("artifact_kind") != EXECUTION_REQUEST_ARTIFACT_KIND:
        return None, {"error": "Invalid real idea answer continuation execution request state artifact_kind"}
    if raw.get("schema_version") != EXECUTION_REQUEST_SCHEMA_VERSION:
        return None, {"error": "Unsupported real idea answer continuation execution request schema_version"}
    if raw.get("state_owner") != "SpecSpace":
        return None, {"error": "Real idea answer continuation execution request state must be owned by SpecSpace"}
    expanded = _first_true(raw, FALSE_FIELDS)
    if expanded is not None:
        return None, {"error": f"Real idea answer continuation execution state cannot claim {expanded}"}
    expanded = _first_true(_record(raw.get("consumer_boundary")), FALSE_FIELDS)
    if expanded is not None:
        return None, {"error": f"Real idea answer continuation execution consumer_boundary cannot claim {expanded}"}
    expanded = _first_true(_record(raw.get("authority_boundary")), AUTHORITY_FALSE_FIELDS)
    if expanded is not None:
        return None, {"error": f"Real idea answer continuation execution authority_boundary cannot claim {expanded}"}
    ...
    *,
    ...
    if expanded is not None:
        return HTTPStatus.BAD_REQUEST, {
            "error": f"Real idea answer continuation execution request payload cannot claim {expanded}",
            "field": expanded,
        }
    ...
    answer_state_ref = _safe_ref(payload.get("answer_state_ref"))
    if answer_state_ref is None:
        return HTTPStatus.BAD_REQUEST, {
            "error": "Missing required request field: answer_state_ref",
            "field": "answer_state_ref",
        }
    intake_execution_ref = _safe_ref(payload.get("intake_execution_ref"))
    if intake_execution_ref is None:
        return HTTPStatus.BAD_REQUEST, {
            "error": "Missing required request field: intake_execution_ref",
    ...
    template_ref = _safe_ref(payload.get("answer_template_ref")) or "runs/real_idea_smoke/real_idea_answer_template.json"
    status = _clean_text(payload.get("status")) or "requested"
    if status != "requested":
        return HTTPStatus.BAD_REQUEST, {
            "error": "New real idea answer continuation execution requests must start as requested.",
            "field": "status",
        }
    now = now_iso()
    request_id = _clean_text(payload.get("request_id")) or (
        f"real-idea-answer-continuation-execute.{workspace_id_value}.{_timestamp_id(now)}.{secrets.token_hex(3)}"
    )
    if not _safe_request_id(request_id):
        return HTTPStatus.BAD_REQUEST, {
            "error": "Invalid real idea answer continuation execution request_id.",
            "field": "request_id",
        }
    ...
    with _STATE_LOCK:
        ...
        record = {
            ...
            "answer_template_ref": template_ref,
...
def _normalize_existing_request(entry: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if expanded is not None:
        return None
    answer_state_ref = _safe_ref(entry.get("answer_state_ref"))
    intake_execution_ref = _safe_ref(entry.get("intake_execution_ref"))
    workspace_initialization_ref = _safe_ref(entry.get("workspace_initialization_ref"))
    if answer_state_ref is None or intake_execution_ref is None or workspace_initialization_ref is None:
        return None
    return {
        "request_id": request_id,
        "workspace_id": workspace_id,
        "answer_state_ref": answer_state_ref,
        "answer_template_ref": _safe_ref(entry.get("answer_template_ref")) or "runs/real_idea_smoke/real_idea_answer_template.json",
        "intake_execution_ref": intake_execution_ref,
        "workspace_initialization_ref": workspace_initialization_ref,
...
def _refresh_summary(
    ...
    *,
    ...
    if requested:
        status = "real_idea_answer_continuation_execution_requested"
        next_gap = "run_platform_real_idea_answer_continuation_execution"
    elif blocked:
        status = "real_idea_answer_continuation_execution_blocked"
        next_gap = "repair_real_idea_answer_continuation_execution_request"
    elif consumed:
        status = "real_idea_answer_continuation_execution_consumed"
        next_gap = "request_real_idea_answer_continuation_execution"
    else:
        status = "no_real_idea_answer_continuation_execution_requests"
        next_gap = "request_real_idea_answer_continuation_execution"
