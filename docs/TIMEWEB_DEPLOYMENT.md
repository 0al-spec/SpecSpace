...
The branch is generated by CI and is manifest-only. Platform renders the
Timeweb manifest from digest-pinned SpecSpace image refs; SpecSpace CI publishes
the rendered tree to the deploy branch. The branch contains the Timeweb root
`docker-compose.yml`, a short `README.md`, and Platform manifest metadata;
application source remains on `main`.
