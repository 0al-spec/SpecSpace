...
Enable Timeweb's automatic deployment on new commits for that branch. The
normal production path is:

```text
main merge -> CI publishes timeweb-deploy commit -> Timeweb auto deploys it
```

The branch is generated by CI and is manifest-only. SpecSpace CI builds and
pushes the API/UI images, writes a `platform_service_image_lock` handoff
artifact with digest-pinned image refs, asks Platform to render the Timeweb
manifest from that lock, and then publishes the rendered tree to the deploy
branch. The branch contains the Timeweb root `docker-compose.yml`, a short
`README.md`, and Platform manifest metadata; application source remains on
`main`.
...
branch after `main` succeeds. SpecSpace still builds and publishes the API/UI
images, while Platform owns the Timeweb manifest shape:
