...
def test_route_table_resolves_real_idea_answer_continuation_execute() -> None:
    post_route = routes.route_for("POST", "/api/v1/real-idea-answer-continuation/execute")

    assert post_route is not None
    assert post_route.handler == "handle_v1_real_idea_answer_continuation_execute_post"
    assert post_route.pass_parsed is True


def test_route_table_resolves_repair_rerun_request_gate_execute() -> None:
    post_route = routes.route_for(
        "POST", "/api/v1/idea-to-spec-repair-rerun-request-gate/execute"
    )
...
def test_route_table_resolves_repair_rerun_publish() -> None:
    post_route = routes.route_for(
        "POST", "/api/v1/idea-to-spec-repair-rerun/publish"
    )

    assert post_route is not None
    assert post_route.handler == "handle_v1_idea_to_spec_repair_rerun_publish_post"
    assert post_route.pass_parsed is True
...
def test_route_table_resolves_candidate_approval_execute() -> None:
    route = routes.route_for("POST", "/api/v1/idea-to-spec-candidate-approval/execute")

    assert route is not None
    assert route.handler == "handle_v1_idea_to_spec_candidate_approval_execute_post"
    assert route.pass_parsed is True
...
def test_route_table_resolves_promotion_execute() -> None:
    route = routes.route_for("POST", "/api/v1/idea-to-spec-promotion/execute")

    assert route is not None
    assert route.handler == "handle_v1_idea_to_spec_promotion_execute_post"
    assert route.pass_parsed is True
...
def test_route_table_resolves_promotion_review_confirmation_authoring() -> None:
    route = routes.route_for(
    ...
    assert (
        route.handler
        == "handle_v1_idea_to_spec_promotion_review_confirmation_post"
    )
    assert route.pass_parsed is True
    assert route.access == routes.RouteAccess.OPERATOR
...
def test_route_table_resolves_review_status_execute() -> None:
    ...
    assert route is not None
    assert route.handler == "handle_v1_idea_to_spec_review_status_execute_post"
    assert route.pass_parsed is True
