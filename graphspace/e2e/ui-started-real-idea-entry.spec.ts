...
const e2eAppOrigin = new URL(
  process.env.SPECSPACE_E2E_BASE_URL ??
    `http://127.0.0.1:${process.env.SPECSPACE_E2E_PORT ?? "5175"}`,
).origin;
const e2eOperatorUsername = "operator";
const e2eOperatorPassword = "specspace-e2e-operator-password-0123456789";
...
async function startSpecSpaceBackend(options: {
  ...
  const operatorAuthEnabled =
    options.enableOperatorAuth ??
    Boolean(options.enablePlatformExecution || options.hostedExecutorUrl);
  const requestHeaders = operatorAuthEnabled
    ? {
        Authorization: `Basic ${Buffer.from(
          `${e2eOperatorUsername}:${e2eOperatorPassword}`,
        ).toString("base64")}`,
        Origin: e2eAppOrigin,
      }
    : {};
      ...
      env: {
        ...
        ...(operatorAuthEnabled
          ? {
              ...
              SPECSPACE_OPERATOR_AUTH_USERNAME: e2eOperatorUsername,
              SPECSPACE_OPERATOR_AUTH_PASSWORD: e2eOperatorPassword,
              SPECSPACE_OPERATOR_AUTH_ALLOWED_ORIGIN: e2eAppOrigin,
            }
          : {}),
...
async function proxyRouteToBackend(
  route: Route,
  baseUrl: string,
  requestHeaders: Record<string, string> = {},
) {
...
async function installRealBackendApiRoutes(
  page: Page,
  baseUrl: string,
  requestHeaders: Record<string, string> = {},
) {
  await page.route("**/api/v1/**", async (route) => {
    ...
    await proxyRouteToBackend(route, baseUrl, requestHeaders);
  });
}
...
async function writeFakePlatformForWorkspaceInitialization(root: string) {
  ...
  await writeFile(
    ...
    [
      ...
      "if 'product-real-idea-continuation' in sys.argv:",
      "    report = {",
      ...
      "        'target_make': {'target': 'real-idea-intake-continue-from-specspace-answers'},",
      "        'summary': {",
      ...
      "                'status': 'succeeded',",
      "                'evidence': ['real-idea-intake-continue-from-specspace-answers'],",
      "            }",
      "        ],",
      "        'output_artifacts': {",
      "            'import_preview': {",
      "                'path': 'runs/pantry-rotation/specspace_real_idea_answer_import_preview.json',",
      "                'present': True,",
      "                'ready': True,",
      ...
      "            },",
      "            'continuation_report': {",
      "                'path': 'runs/pantry-rotation/real_idea_answer_continuation_report.json',",
      "                'present': True,",
      "                'ready': True,",
      ...
      "        },",
      "    }",
      "    workspace_dir = output.parent / 'pantry-rotation'",
      "    workspace_dir.mkdir(parents=True, exist_ok=True)",
      "    (workspace_dir / 'real_idea_smoke').mkdir(parents=True, exist_ok=True)",
      "    clarification = {",
      "        'artifact_kind': 'idea_to_spec_clarification_requests',",
      "        'schema_version': 1,",
      "        'contract_ref': 'specgraph.idea-to-spec.clarification-requests.v0.1',",
      ...
      "                'accepted_actions': ['answer_question', 'defer'],",
      "                'suggested_answer_shape': 'refs[]',",
      "                'value_templates_by_action': {'answer_question': {'refs': ['']}, 'defer': {'follow_up': ''}},",
      "                'required_fields_by_action': {'answer_question': ['value.refs[]'], 'defer': ['value.follow_up']},",
      "            }",
      "        ],",
      "        'readiness': {'ready': True, 'review_state': 'answer_template_ready', 'blocked_by': []},",
      "        'authority_boundary': {",
      "            'may_execute_specgraph': False,",
      "            'may_write_ontology_package': False,",
      "            'may_accept_ontology_terms': False,",
      "        },",
      "    }",
      "    (workspace_dir / 'idea_intake_clarification_requests.json').write_text(json.dumps(clarification), encoding='utf-8')",
      "    (workspace_dir / 'real_idea_smoke' / 'real_idea_answer_template.json').write_text(json.dumps(template), encoding='utf-8')",
...
function safeManagedModeBoundary() {
  return {
    ...safeActionBoundary(),
...
function safePromotionReviewConfirmationBoundary() {
  return {
    report_only: true,
...
function managedModeReadinessForScenario(scenario: UiStartedIdeaScenario) {
  ...
  return {
    ...
    authority_boundary: safeManagedModeBoundary(),
  };
}

function promotionReviewConfirmationForScenario(
  scenario: UiStartedIdeaScenario,
) {
  const ready = scenario.promotionReviewConfirmationReady === true;
  return {
    artifact_kind: "specspace_promotion_review_confirmation_authoring",
    schema_version: 1,
    available: true,
    status: ready ? "ready" : "confirmation_needed",
    workspace_id: workspaceId,
    ...
    confirmation: {
      ...
      confirmation_id: ready
        ? `confirmation://${workspaceId}/promotion_review_execute/0123456789abcdef0123456789abcdef`
        : null,
      issued_at: ready ? "2026-08-02T00:00:00Z" : null,
      expires_at: ready ? "2026-08-02T00:10:00Z" : null,
      predecessor_request_id: ready
        ? `managed-operation://${workspaceId}/promotion_execute_dry_run/0123456789abcdef01234567`
        : null,
    },
    bound_inputs: [],
    blockers: [],
    next_safe_action: ready
      ? "Request the separately allowlisted review operation."
      : "Authorize review creation for the current successful promotion dry-run.",
    authority_boundary: safePromotionReviewConfirmationBoundary(),
  };
}
...
async function workspacePayload(
  ...
  const managedOperations = managedOperationsObservabilityForScenario(
    scenario.overviewStage ?? defaultOverviewStageForScenario(scenario),
  );
  if (scenario.approvalPath) {
    ...
    managedOperations.operations.push({
      ...
      operation_id: "promotion_review_execute",
      category: "promotion",
      lifecycle_stage: "promotion_review_execution",
      ui_stage: "Promotion review",
      endpoint: "/api/v1/idea-to-spec-promotion-review/execute",
      platform_command: ["product-promotion", "execute"],
    ...
    managedOperations.summary.operation_count += 1;
    if (promotionReviewStatus === "ready_to_execute") {
      managedOperations.summary.ready_to_execute_count += 1;
    } else {
      managedOperations.summary.request_needed_count += 1;
    }
    managedOperations.status_counts[promotionReviewStatus] =
      (managedOperations.status_counts[promotionReviewStatus] ?? 0) + 1;
    managedOperations.groups.push({
      phase: "promotion",
      label: "Promotion",
      operation_ids: ["promotion_review_execute"],
    });
  }
...
async function installIdeaToSpecApiRoutes(
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path === "/api/v1/idea-to-spec-promotion-review-confirmation") {
      ...
      scenario.promotionReviewConfirmationReady = true;
      await route.fulfill({
        status: 201,
        json: {
          ok: true,
...
async function expectProductWorkspaceOverviewStage(
  ...
  await expect(page.getByTestId("managed-mode-readiness")).toContainText("read only");
  await expect(page.getByTestId("managed-mode-readiness")).toContainText(
    "Execution unavailable: platform_execution_disabled",
  );
...
test("runs workspace initialization through SpecSpace backend managed execution", async ({
  ...
  try {
    await installRunsWatchMock(page);
    await installRealBackendApiRoutes(
      page,
      backend.baseUrl,
...
test("runs guided approval managed actions through backend endpoints", async ({
  ...
  try {
    ...
    await expect(guidedApprovalPath).toContainText(
      "Promotion review authorization: ready",
    );
    await expect(guidedApprovalPath).toContainText("Open review PR");
    await expect(
      guidedApprovalPath.getByRole("button", { name: "Open review PR" }),
    ).toBeEnabled();
    await guidedApprovalPath.getByRole("button", {
      name: "Open review PR",
    }).click();
...
test("product demo harness: UI-started real idea reaches candidate with explicit fallback policy", async ({
  ...
  try {
    ...
    const workspaceResponse = await fetch(
      ...
      { headers: backend.requestHeaders },
    );
...
test("shows continuation-ready lane after external answer continuation publication", async ({
  ...
  try {
    ...
    await page.route("**/api/v1/real-idea-answer-continuation/execute**", async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page
      ...
      .click();
    await expect(
      page.getByTestId("guided-clarification-continuation-managed-execute-status"),
    ).toContainText("managed_real_idea_answer_continuation_executed");
