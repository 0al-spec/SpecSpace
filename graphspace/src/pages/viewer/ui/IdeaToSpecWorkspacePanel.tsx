...
export function IdeaToSpecWorkspacePanel({
  ...
  const executePromotionDryRun = async () => {
    const workspaceId = data.selectedWorkspaceId ?? data.workspace.id;
    if (readOnly || !promotionExecuteUrl || !workspaceId) {
      return;
    }
    setPromotionExecutionState({
      pending: true,
      status: null,
      error: null,
    });
    try {
      const response = await fetch(promotionExecuteUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workspace_id: workspaceId }),
      });
      ...
      if (!response.ok) {
        const detail =
          typeof body.error === "string"
            ? body.error
            : `Managed promotion execution failed with HTTP ${response.status}.`;
        setPromotionExecutionState({
          pending: false,
          status: typeof body.status === "string" ? body.status : null,
          error: detail,
      ...
      setPromotionExecutionState({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Managed promotion execution failed.",
      });
    }
  };
  ...
  const intakeClarificationSection = (
    <IntakeClarificationSection
      ...
      workspaceInitializationRef={
        data.workspaceInitializationPath.initializationReportRef
      }
  ...
  return (
    ...
    >
      ...
      <div className={styles.row}>
        ...
        {!initializationRequestRef && path.status !== "initialized" ? (
...
function IdeaIntakeDraftSection({
  ...
  const workspaceInitializationRef =
    workspaceInitializationPath.initializationReportRef;
  const canRequestIntakeExecution =
    !readOnly &&
    activeEntry !== null &&
    workspaceInitialized &&
    workspaceInitializationRef !== null &&
        ...
        {entryHandoffCommand ? (
          <div className={styles.row}>
            ...
            <button
              ...
              onClick={() => {
                if (!activeEntry || workspaceInitializationRef === null) return;
  ...
  return (
    <section id="idea-to-spec-guided-approval-path" className={styles.reviewSection}>
      ...
      <div className={styles.row}>
        ...
        {showManagedPromotionExecution ? (
          <div className={styles.draftControls}>
            <button
              className={styles.ackButton}
              type="button"
              disabled={!canRunManagedPromotionExecution}
              onClick={onPromotionExecute}
            >
              {promotionExecutionState?.pending
                ? "Running promotion dry-run..."
                : "Run promotion dry-run"}
            </button>
            <span className={styles.statusDetail}>
        ...
        {promotionExecutionState?.status ? (
          <span className={styles.statusDetail}>
            Promotion execution: {promotionExecutionState.status}
          </span>
        ) : null}
...
function IntakeClarificationSection({
  ...
  workspaceInitializationRef: string | null;
  executeUrl?: string;
  onWorkspaceRefreshRequest?: () => void;
  readOnly: boolean;
}) {
  ...
  const continuationRequestReady =
    ...
    workspaceInitialized &&
    workspaceInitializationRef !== null &&
    intakeExecutionRef !== null &&
    answerTemplateRef !== null &&
    lane.available &&
