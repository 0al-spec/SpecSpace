...
export function useProposalIndex(options: Options = {}): UseProposalIndexState {
  const { url = "/api/v1/proposals", fetcher = fetch, refreshKey = 0 } = options;
  const [state, setState] = useState<UseProposalIndexState>({ kind: "idle" });
  ...
  useEffect(() => {
    ...
    fetcher(url, { signal: controller.signal })
      .then(async (response) => {
        ...
        const parsed = parseProposalIndex(payload);
